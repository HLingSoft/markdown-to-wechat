...
const prefix = `torra-markdown-to-wechat`
...
export function postProcessHtml(
  ...
  opts?: { primaryColor?: string; codeThemeHref?: string }
): string {
  // 先基于 baseHtml 做清洗/补丁
  let html = baseHtml

  // ① code：没有 .hljs 的给它补上；行内 <code> 没 class 也补一个
  html = html.replace(
    /<code class="(?![^"]*\bhljs\b)([^"]*)"/g,
    (_m, cls) => `<code class="hljs ${cls.trim()}"`,
  )
  html = html.replace(/<code(?![^>]*class=)/g, '<code class="hljs"')

  // ② blockquote：去掉多余包裹的 p（有些扩展会残留）
  html = html.replace(
    /<blockquote([^>]*)>\s*<p([^>]*)>([\s\S]*?)<\/p>\s*<\/blockquote>/g,
    (_m, bAttr, pAttr, inner) => `<blockquote${bAttr}><p${pAttr}>${inner}</p></blockquote>`,
  )

  // ③ 注入主题主色（可选）
  const vars = opts?.primaryColor
    ? `<style id="md-theme-vars">:root{--md-primary-color:${opts.primaryColor};}</style>`
    : ``

  // ④ 注入代码高亮主题（可选）
  const hljsLink = opts?.codeThemeHref
    ? `<link id="hljs" rel="stylesheet" href="${opts.codeThemeHref}">`
    : ``

  // ⑤ 组装最终 HTML
  const addition = renderer.buildAddition?.() ?? ''      // 你自带的样式补充
  const reading = renderer.buildReadingTime?.(rt) ?? '' // 阅读统计（按开关可能为空）
  const footnotes = renderer.buildFootnotes?.() ?? ''

  // ⚠️ 用“处理后的” html，而不是 baseHtml
  const body = renderer.createContainer(html)

  return `${vars}${hljsLink}${addition}${body}${reading}${footnotes}`
}
